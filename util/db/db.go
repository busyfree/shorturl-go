...
import (
	...
	"github.com/busyfree/shorturl-go/util/errors"
...
func Get(ctx context.Context, name string) *DB {
	...
	maxOpenConns := cast.ToInt(config["max_open_conns"])
	maxIDLEConns := cast.ToInt(config["max_idle_conns"])
	maxConLifeDuration := cast.ToDuration(config["max_life"])
	if maxOpenConns == 0 {
		maxOpenConns = 20
	}
	if maxIDLEConns == 0 {
	...
	if maxConLifeDuration > 0 {
		maxConLifeDuration = maxConLifeDuration * time.Second
	} else {
		maxConLifeDuration = time.Duration(60) * time.Second
	}
	sqldb.SetMaxOpenConns(maxOpenConns)
	sqldb.SetMaxIdleConns(maxIDLEConns)
	sqldb.SetConnMaxLifetime(maxConLifeDuration)
