...
import (
	"fmt"
	"github.com/busyfree/shorturl-go/util/check"
	"github.com/busyfree/shorturl-go/util/randutil"
	"io"
	"io/ioutil"
	"os"
	"path/filepath"
...
func init() {
	...
	saveMachineIDToConfigFile()

	viper.AutomaticEnv()
}

func saveMachineIDToConfigFile() {
	machineID := cast.ToUint16(GetInt("SNOWFLAKE_MACHINE_ID"))
	path := filepath.Join(GetConfigPath(), "config.toml")
	if machineID == 0 {
		machineID = GetEnvMachineId()
		if machineID > 0 {
			fmt.Println("got machineID from os env")
		}
		hostName, _ := os.Hostname()
		if len(hostName) > 0 {
			re := regexp.MustCompile("[0-9]+")
			numberStrArr := re.FindAllString(hostName, -1)
			if len(numberStrArr) > 0 {
				numberStr := strings.Join(numberStrArr, "")
				machineID = cast.ToUint16(numberStr)
				if machineID > 0 {
					fmt.Println("config.toml SNOWFLAKE_MACHINE_ID not defined,so got machineId:", machineID, " from HostName AND save to config.toml")
				}
			}
		}
		if machineID == 0 {
			machineID = randutil.RandUint16(60000, 65500)
			fmt.Println("config.toml SNOWFLAKE_MACHINE_ID not defined,so use rand value:", machineID, " AND save to config.toml")
		}
		if !check.CheckFileExist(path) {
			_ = ioutil.WriteFile(path, []byte(fmt.Sprintf("SNOWFLAKE_MACHINE_ID=%d", machineID)), os.ModePerm)
		} else {
			fWriter, openFileErr := os.OpenFile(path, os.O_APPEND|os.O_WRONLY, os.ModeAppend) //打开文件
			if openFileErr != nil {
				panic(fmt.Sprintf("open %s file error", path))
			}
			defer fWriter.Close()
			_, writeFileErr := io.WriteString(fWriter, fmt.Sprintf("\n# snowflake \nSNOWFLAKE_MACHINE_ID=%d\n", machineID))
			if writeFileErr != nil {
				panic(fmt.Sprintf("write %s file error:%v", path, writeFileErr))
			}
		}
	}
	viper.Set("SNOWFLAKE_MACHINE_ID", machineID)
}

func GetEnvMachineId() uint16 {
	return MachineID
}
